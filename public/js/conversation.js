...
var ConversationPanel = (function() {
  ...
  function setupInputBox() {
    ...
    function adjustInput() {
      if (input.value === '') {
        ...
        if (currentFontSize) {
          padding = Math.floor(
            (currentFontSize - minFontSize) / (maxFontSize - minFontSize) * (maxPadding - minPadding) + minPadding
          );
        } else {
          padding = maxPadding;
        }
  ...
  function displayMessage(newPayload, typeValue) {
    ...
    if (isUser !== null && textExists) {
      ...
      var previousLatest = chatBoxElement.querySelectorAll(
        (isUser ? settings.selectors.fromUser : settings.selectors.fromWatson) + settings.selectors.latest_new
      );
      ...
      if (!isUser) {
        if (Object.keys(newPayload.output).length >= 3) {
          var bubbles = newPayload.output.bubble;
          if (bubbles != undefined) {
            var segment = document.getElementById('scrollingChat');
            var segmentLoad = document.createElement('div');
            ...
            messageInner.appendChild(list);
            var i = 0;
            var li = [];
            while (i < bubbles.length) {
              li[i] = document.createElement('li');
              var t = document.createTextNode(bubbles[i]);
              li[i].appendChild(t);
              li[i].classList.add('bubble-list-item');
              var bubbleItem = bubbles[i];
              li[i].setAttribute('onclick', 'ConversationPanel.clickHandler(event, "' + bubbleItem + '")');
  ...
  function scrollToChatBottom() {
    ...
    var scrollEl = scrollingChat.querySelector(settings.selectors.fromUser + settings.selectors.latest_new);
    if (scrollEl) {
